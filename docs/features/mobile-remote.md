...
| Browser | `node tools/mobile-web-qa/run-qa.mjs`（fake DaemonClient + 真实 SPA 栈；需 `npm i --no-save puppeteer-core` 与 Chrome；48 检查含 Files/Diff/MCP/Skills 工作环、kill-list 零写断言、流事件锚点、openSession 失败占位、已保存电脑 chooser）；`node tools/remote-web-qa/run-e2e.mjs --relay <endpoint>`（真 daemon 子进程 + 真 offer + LAN-IP origin 真浏览器全链路 E2EE 配对 8 检查，含垃圾 offer 失败面与 stopDaemon 端口回收；本地中继复现步骤见 `docs/qa/results/2026-08-28/remote-web-full-test.md`） |
