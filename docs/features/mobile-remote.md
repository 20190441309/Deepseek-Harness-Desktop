...
| **id** | `mobile-remote` |
| **status** | `active` |
| **last verified** | 2026-08-27 — Android 改为应用内 WebView 承载同一 v2 SPA：原生扫码/粘贴严格校验 offer v2，保留 SPA localStorage sticky 重连，并删除不可达的 HTTP v1 原生 Chat 客户端。 |
...
3. Android：原生扫码或粘贴完整配对 URL → 应用内 WebView 打开同一 SPA → 后续启动用已记住的无 fragment 落地 URL 触发 SPA sticky 重连。
...
- Android Compose 扫码框为正方形；会话走应用内 WebView 的同一 Web SPA，不另写一套 DaemonClient。
- Android 升级后一次性清除旧 HTTP `deviceToken`/`origin`；不保留 `LoginClient`、Bearer `/api/*`、`/__remote__/shell/*` 原生 Chat 死路径。
- Android 原生层只保存无 `#offer=` 的 SPA 落地 URL；`deviceSecret` 由 SPA 保存在 WebView origin 的 localStorage，直到桌面撤销或 SPA 断开设备。
- 设计语言仍抄 `--dsw-alias-*`。
...
| Kind | What |
| --- | --- |
| Automated | `mobile/web/host/offer.test.js`、`login.test.js`、`pair/scan.test.js`、`chisacode/session.test.js`；Android `OfferTest` / `DshViewModelTest`；`src/shared/lan.test.js`；`chisacode-remote.test.js` |
