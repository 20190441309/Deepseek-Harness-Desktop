...
| **id** | `session-archive` |
| **status** | `active` |
| **last verified** | 2026-08-26 — 已归档每次加载默认折叠（展开不持久化）；点行无动作；删除 harden 仍在 |
...
- 删除成功：先发 `host/session-deleted*`，再 unarchive；unary ok 时装归档回声并 `applyDeleted`；对话框仍仅当归档集不再含该 id 时关闭。
...
| Kind | What |
| --- | --- |
| Automated | vendor workspace + apiproxy `api-proxy-session-delete` + runtime workspaces/sessions + `pnpm run test:gui`（ui-workspace）；可见 UI 另跑 `DSH_SNAPSHOT=replay pnpm run test:web` |
