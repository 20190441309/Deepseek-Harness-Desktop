...
- skip compose 语义由打包门禁背书：after-pack 对打进 dist 的真实 CLI 跑 `dump-config` skip/full 双轮（`scripts/check-skip-compose-contract.js`，临时 DSH_HOME + 用户层 canary + 预埋受管块回放迁移）——skip 轮 canary 必须消失且 install 插件仍挂载，full 轮 canary 必须回来；两轮都传 overlay（对齐生产 argv），install 行必须**恰好一次**（>1 = 受管块残留与 overlay 双挂载），迁移后 `cordis.patch.yml` 不得再含受管记号且 canary 用户行必须保留；任一违反即打包失败。单测层不能替代此门禁（单测 mock 了 dsh.start）。
...
- `src/main/harness-controller.js` 仅限 sticky skip 判定委托与启动 `patchFiles` 组装（install overlay 所有启动、usage overlay 仅全量；不动其余生命周期语义）
- skip 救生链路（2026-08-26 扩入，理由：skip 语义由这些文件共同实现）：`src/main/dsh.js`（`buildLaunch` argv/预检、`harnessSpawnPlan` 引号）、`src/main/plugins.js`（`ensureDesktopInstallPlugin` overlay + 受管块迁移）、`src/main/usage-panel-preset.js`（overlay 化，行为语义归 usage-stats 卡）、`src/main/plugin-tree-failure.js`、`src/main/plugin-runtime-files.js`、`src/shared/launcher-recovery.js`
