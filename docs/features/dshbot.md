...
| P1 | **独立仓拆分 + npm 发布** `ChisaAlter/dshbot` / `dshbot@<semver>`：拆分交付件已就绪（2026-08-28）——`scripts/export-dshbot-standalone.mjs` 确定性导出独立仓树（根布局、`repository`/`homepage` 指新仓、`v<semver>` tag 触发独立 `publish.yml`、根布局预检 `check-publish.mjs` 含 repository guard 防错仓发布），种子分支 `cursor/dshbot-standalone-seed-f2c5`（单干净提交）已推 monorepo 且 `npm install github:…#<seed>` 实测装通；monorepo 内 tag `dshbot-v<semver>` 链路（`publish-dshbot.yml` + `check-dshbot-publish.mjs`）保留可用。**剩余缺口（都需要仓主）：** ① 用用户级凭证建空仓 `ChisaAlter/dshbot`（云端 integration token 403，建不了个人仓），把种子推为 main：`git push git@github.com:ChisaAlter/dshbot.git cursor/dshbot-standalone-seed-f2c5:main`；② 在（新仓或 monorepo）配 `NPM_TOKEN` secret 后推 tag（新仓 `v0.2.0` / monorepo `dshbot-v0.2.0`）即发 | 首个 tag 发布成功；README / 发布说明写清 `dsh plugin add dshbot@x.y.z` 规格；独立仓上线后第一方行安装规格切 `github:ChisaAlter/dshbot`（或 registry 收录后让位） |
