...
| **id** | `data-import` |
| **status** | `active` |
| **last verified** | 2026-08-24 — 扫描排除 `_no-cwd/preset-*` harness 预设会话；折叠用 `li[hidden]` 生效；列表读 session header/`session/title` 做人名/cwd 展示 |
...
3. 导入页用分类页签勾选会话 / 技能 / 插件 / MCP，默认可导入项全勾。会话按工作区分组：展示真实 `cwd` 或「无工作区」、对话标题（否则 id）；≥8 项的组默认折叠。扫描**不列出** harness 预设夹 `_no-cwd/preset-*`（非用户对话）。空选点导入 = 不写盘。落点固定桌面 `userData/dsh-home`。
