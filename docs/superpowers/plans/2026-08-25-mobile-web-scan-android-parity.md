...
- [ ] `docs/features/mobile-remote.md`：`status` 改 `parked`（与 README 索引、QA N/A 一致），User paths 标注「停放期间入口隐藏」；Allowed touch 已含 `mobile/web/`，无需扩权
- [ ] 卡内 Gates 增补本计划新增测试文件名；Sources 补本计划链接
- [ ] `docs/handbook/modules/mobile-remote.md` 延伸阅读补本计划
