...
2. **调度：** Host 从最近用户消息解析 `@名` / `@everyone|@all`；否则全员。`orderRoundSpeakers` 按轮旋转；成员为 2–6 个，硬顶 `maxSpeaks=10`、`maxRounds=3`、每成员每回合最多 2 条投递。一轮全 pass 则停。`maxSpeaks` 按 Grok 语义**只计可见投递**（2026-08-27 修正，此前短暂按尝试计）：pass、成员失败与重启后无结果调用不耗额度，只推进轮转队列，总尝试由全 pass 停轮 + `maxRounds` 封顶。同 turn 两条投递是两条独立可见消息。不再用 `NEXT:` 脚注决定下一说话人（旧日志仍可剥离显示）。
