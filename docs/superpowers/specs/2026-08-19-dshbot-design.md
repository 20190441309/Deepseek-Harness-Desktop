...
2. **调度：** Host 从最近用户消息解析 `@名` / `@everyone|@all`；否则全员。`orderRoundSpeakers` 按轮旋转；成员为 2–6 个，硬顶 `maxSpeaks=10`、`maxRounds=3`、每成员每回合最多 2 条投递。一轮全 pass 则停。`maxSpeaks` 计 `ask_participant` 尝试，pass、成员失败与重启后无结果调用都消耗一次；不按可见投递计。不再用 `NEXT:` 脚注决定下一说话人（旧日志仍可剥离显示）。
