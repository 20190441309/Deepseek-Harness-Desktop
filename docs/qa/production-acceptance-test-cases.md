...
**期望：** 归档只隐藏列表，日志仍在；取消归档后行恢复，可正常打开续聊。永久删除日志不在本条范围（C2 / TC-CHAT-011）。
...
### TC-EXT-007 · 预置 dshbot 隐藏 · P0

**步骤：**

1. 确认侧栏没有「机器人 / Bots」页签（文案以界面为准）。  
2. 设置 → 市场「已安装」中 dshbot 不是必现目录行。  

**期望：** 没有机器人入口；启动不因卸掉预置失败。仓库仍带 `vendor/dshbot`，不以「安装包缺源」判 Fail。
...
**步骤：** 关窗进托盘。Win11 须先打开托盘溢出（「显示隐藏的图标」）再 **右键** 托盘图标，逐项：**显示窗口**、**设置**、**插件市场**、**重启 Harness**、**退出**；单击托盘图标。未展开溢出就放弃 ≠ 产品缺菜单（`tray-menu.js` 已有这五项）。
