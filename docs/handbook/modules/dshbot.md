...
- 房间推进借 Harness `llm/stream` → 链式 `ask_participant`；成员 spawn `toolFilter` 仅 `send_room_message`，system prompt 同口径（talking-circle）。10 次成员上限计**可见投递**（pass/失败不消耗）；非取消失败变成静默 pass。Config 只接受整数 `maxSpeaks=1..10`、`maxRounds=1..3`；`send_to_agent` 仅允许 post 到所属群；idle room 帖在下一轮用户消息 drain。
...
- Automated：`dshbot-*.test.js`（含 room-preset 自装、market-row 目录行与安装通道、runtime-resilience epoch/inbox、协议失败/pass/尝试硬顶/2–6 成员、pnpm 软链保护、UI 清理、prompt/toolFilter 一致、avatar lockstep、publish-manifest 发布完整性）、`harness-controller`、`release-ui-walk` `plugin.dshbot.tabAbsent`
