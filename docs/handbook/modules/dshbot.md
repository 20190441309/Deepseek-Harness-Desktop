...
- 默认无 Bots 页签。设置 → 插件市场第一方行一键安装，或 `dsh plugin add`（独立仓 `github:ChisaAlter/dshbot`；旧 `github:ChisaAlter/Deepseek-Harness-Desktop#path:/vendor/dshbot` 已退位）；装后建 Bot、建群（名称 + description + 2–6 成员）、打开 1:1 或群；卸载后页签消失，桌面启动清无主 preset。
