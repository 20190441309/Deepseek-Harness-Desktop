...
由 PTC `write`/`edit`/`str_replace_editor` 成功创建或修改的文件现在进入所在轮次的产出词表，收尾正文里的精确路径或唯一 basename 会渲染为现有代码芯片，点击经 `workspaces.openPath` 打开右侧 Sidebar 文档预览；重复打开复用同一 tab。PTC 失败、读取、查看、未知工具和格式错误参数不贡献条目；两条同名路径的 basename 仍保持惰性，避免打开错误文件。
