...
- **品牌徽标换新**：应用图标、托盘图标与安装器标统一换成鲸鱼娘大头——立绘居中裁剪、圆角深色徽标，告别黑底白鲸鱼；启动页加载换成鲸鱼娘奔跑动画（支持减少动态效果时显示静帧）。
- **Harness 基线 dsh-v0.1.6-alpha.1**：桌面客户端与安装包升级到同一套新的钉版基线；官方 DeepSeek 端点归一到 Messages 协议根 `https://api.deepseek.com/anthropic`，第三方网关地址不受影响。
