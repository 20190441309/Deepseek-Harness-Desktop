#!/usr/bin/env node
// Publish preflight for the standalone dshbot plugin (vendor/dshbot).
//
// Usage:
//   node scripts/check-dshbot-publish.mjs               # manifest integrity only
//   node scripts/check-dshbot-publish.mjs dshbot-v0.2.0 # + tag/version match
//
// The release workflow (.github/workflows/publish-dshbot.yml) runs this before
// `npm publish`; src/main/dshbot-publish-manifest.test.js locks the same
// invariants in the desktop unit suite so a broken manifest fails CI early.
...
if (tag) {
  const expected = `dshbot-v${pkg.version}`
  if (tag !== expected) {
    errors.push(`tag ${tag} does not match vendor/dshbot version ${pkg.version}; expected ${expected}`)
  }
}
