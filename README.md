...
5.支持标题栏 Git：有改动时主按钮为 Commit & push，下拉可只 Commit。
6.支持底栏终端，以及右侧栏五卡（文件 / 差异 / 终端 / 浏览器 / 代理）。
...
- **标题栏 Git**：有工作区改动时主按钮为 Commit & push；下拉可只 Commit，默认分支会确认
- **底栏终端与右侧栏**：标题栏开关打开底栏 PTY 和右侧五卡（文件 / 差异 / 终端 / 浏览器 / 代理）；底栏与右侧终端共用同一会话族。工具详情仍在中间偏右的详情列，与右侧栏互不影响
