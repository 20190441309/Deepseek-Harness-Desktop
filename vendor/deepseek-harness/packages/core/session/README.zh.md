...
- `session.deriveMessages()` 对每个新的 surface 条目只做一次增量投影，并返回一个新数组，其中包含这些条目存储的完整、带标识且冻结的消息。assistant 消息的模型来源会保留生成该消息的提供方和模型，以及适配器私有回放状态。投影会移除格式错误的工具调用（ID 为空或名称不匹配 `[A-Za-z0-9_-]{1,64}`）、抑制其结果，并从被修改的 assistant 消息中删除回放元数据；仅追加日志不会被改写。surface 重写会重建投影；不存在原始日志回退。
