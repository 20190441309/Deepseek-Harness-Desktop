...
- `session.deriveMessages()` incrementally projects each new surface entry once and returns a fresh array over the complete identified, frozen messages stored by those entries. Assistant messages preserve the provider and model that produced them plus adapter-private replay state in their model source. Projection removes malformed tool calls (empty id or a name outside `[A-Za-z0-9_-]{1,64}`), suppresses their results, and drops replay metadata from a changed assistant message; the append-only log is not rewritten. A surface rewrite rebuilds the projection; there is no raw-log fallback.
