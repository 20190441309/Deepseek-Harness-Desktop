...
- A plain browser still has no built-in General rows. The desktop close-window and remote-access rows appear only when the Electron shell can persist them; every other row still arrives from its owning feature plugin.
