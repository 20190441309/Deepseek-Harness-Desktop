...
Declared children are all `single` + `session-maybe`: `surfaces.browser`, `surfaces.terminal`, `surfaces.files`, `surfaces.file`, `surfaces.diff`, `surfaces.agents`. `surfaces.terminal` matches the ui-user-terminal inject so the existing Terminal occupant attaches. `surfaces.files` owner is `openFile(relativePath)`; `surfaces.file` owner is `relativePath`. The Diff empty-state card is disabled when `gitStatus(cwd)` is null, with the T3code reason `Diff is only available for server threads in Git repositories.` The Browser card is disabled when desktop `window.shell.previewOpen` is absent, with the T3code reason `Browser previews are only available in the desktop app.` Occupant bodies are injected by later packages.
