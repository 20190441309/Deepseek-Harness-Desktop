...
- **Marketplace is desktop-only** — the tab is absent in a plain browser. One-click install on the Settings tab confirms the catalog spec in a Modal, then calls the desktop shell by catalog id. Uninstall still calls the desktop shell, which forwards to `dsh plugin --profile web remove` and restarts the Host.
