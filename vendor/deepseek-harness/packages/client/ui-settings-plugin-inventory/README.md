...
- **Marketplace is desktop-only** — the tab is absent in a plain browser. One-click install on the Settings tab confirms the catalog spec in a Modal, then calls the desktop shell by catalog id. If add succeeds and Harness does not start, the tab shows that the plugin is already in the web profile and does not add it again. If remove succeeds and Harness does not start, the tab shows that the plugin is already out of the web profile and does not remove it again. Uninstall still calls the desktop shell, which forwards to `dsh plugin --profile web remove` and restarts the Host.
