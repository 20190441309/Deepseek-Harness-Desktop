...
- **插件市场仅桌面端** —— 普通浏览器不注册该标签页。设置页一键安装在 Modal 中确认目录规格后，按目录 id 调用桌面 shell。若写入成功但 Harness 没有起来，标签页说明插件已在 web profile 里，不再安装一次。若卸载成功但 Harness 没有起来，标签页说明插件已从 web profile 移除，不再卸载一次。卸载仍走桌面 shell，转发为 `dsh plugin --profile web remove`，完成后重启 Host。
