...
| `delete(id): Promise<void>` | 删除一个会话的持久日志。未知 id 拒绝。未实体化的 create 会取消并成功返回。成功后该 id 对 `load`/`list` 未知。 |
...
| `deleteStored(id)` | 删除一个已实体化会话的持久字节。协调器在该 id 的写入链上串行化此操作。 |
...
- **无保留策略或附件 GC**：`delete` 删除一个会话的持久日志。附件 blob 和 message-feedback sidecar 不会被回收。
