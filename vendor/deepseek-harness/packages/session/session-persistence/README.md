...
| `delete(id): Promise<void>` | Remove one session's durable log. Unknown id rejects. An un-materialized create cancels and resolves. After success the id is unknown to `load`/`list`. |
...
| `deleteStored(id)` | Remove one materialized session's durable bytes. The coordinator serializes this on the per-id write chain. |
...
- **No retention or blob GC API** — `delete` removes one session's durable log. Attachment blobs and message-feedback sidecars are not collected.
