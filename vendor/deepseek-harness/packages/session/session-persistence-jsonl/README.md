...
- **`delete` removes the session-owned directory** — `sessionDir` is removed recursively; an empty project directory is then best-effort `rmdir`. Attachment blobs are not garbage-collected.
