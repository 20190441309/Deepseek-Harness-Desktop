...
**Reading configuration is as privileged as writing it.** `settings.describe`, `credentials.describe`, every `mcpServers/*` management Remote, and every `skillInventory/*` management Remote join the loopback-only set, so the whole configuration plane stays same-origin until real authentication exists. The slash Typert endpoints are pinned explicitly: they expose local paths and Skill bodies, and can mutate configuration or start stdio commands; `trustedHosts` remains a DNS-rebinding fence, not management authentication. The model catalog (`llm.providers`, `llm.models`) deliberately does not: it carries provider ids, display names, and model lists — no endpoints, no key state — and a LAN client's model picker needs it. The boundary is asserted over a real HTTP server rather than a hand-assembled request, because the `Host` header a browser actually sends is what decides it.
