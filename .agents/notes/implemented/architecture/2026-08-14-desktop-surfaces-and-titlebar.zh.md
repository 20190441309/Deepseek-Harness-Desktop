...
注入脚本只绘制无边框窗口控件板（`#dshd-shell-controls`），并发布 `--dshd-wco-controls`。ConversationRoot 拥有标题栏命中：`.titleRow` 与空白会话的 `.blankCaption` 为 `-webkit-app-region: drag`；可点击的 breadcrumb、`header.actions`、utilities 和 tabs 为 `no-drag`。`<header>` 本身不是拖动区，因为它的 border box 与 `#dshd-shell-titlebar-trailing` 重叠。空白 / 草稿会话仍把该页头留在网格第 1 行，渲染无交互的 caption，因此窗口可拖且不盖住尾簇控件。AppFrame 测量 `#dshd-shell-titlebar-trailing` 以发布 `--dshd-titlebar-conversation-reserve`；注入脚本不测量尾簇。AppFrame 有一行共享标题栏网格（`auto` + 主体 + 抽屉）。会话栏页头与滚动主体是该行对的 subgrid 项（`ConversationRoot` 为 `display: contents`）。详情栏占用主体行，因此分割线和占用者从标题栏带下方开始。surfaces 跨越所有网格行直到窗口顶部。surfaces 打开时，尾簇只占第 2–3 列（`margin-right: 8px`），Session log、Git 和面板开关停在第 4 列之前；窗口控件通过 surfaces 标签栏上的 `margin-right: var(--dshd-wco-controls)` 避让（ui-surfaces），因此标签栏的 `-webkit-app-region: drag` 区域不含控件垫。surfaces 关闭时，尾簇伸到右缘（`margin-right: var(--dshd-wco-controls, 8px)`）。尾簇是该标题栏行的网格项（`justify-self: end`），不是盖在栏内容上的 overlay。手机与 compact-header 框架隐藏尾簇；关闭的列宽度为 0 且不画分割线，因此不会留下空洞。尾簇仍画在被挤窄的会话栏上时，AppFrame 发布 `--dshd-titlebar-conversation-reserve` 以及 `full` / `cozy` / `compact` 密度，让页头让位、标签收起而不是互叠；由[标题栏拥挤笔记](../bug-fix/2026-08-17-titlebar-crowding-density.md)拥有。注入脚本是可重复执行的 IIFE：对同一文件的第二次 `executeJavaScript` 不得抛错。
...
**注入常驻 48px 拖动遮罩。** 超高 z-index 的条带会盖住会话标题、preset 和页签；下层元素的 `no-drag` 无法恢复点击。改由 ConversationRoot 在真实布局中声明互不重叠的 drag / no-drag 区域。
...
各包套件钉住让步、store 动作、标题栏注入 / dispose（资源释放）、Git 状态、独立 PTY 所有权，以及五卡空态。`src/main/harness-chrome-inject.test.js` 钉住 IIFE 形态、二次求值、窗口控件板上的 pointerdown、仅 `--dshd-wco-controls`，以及不存在拖动条、MARK/HIT 属性和 DOM 嗅探。会话栏与 AppFrame CSS 钉住 active / blank / phone / surfaces 打开时互不重叠的 drag 与 no-drag 区域。`apps/web/tests/desktop-chrome.e2e.ts` 是无密钥的组装布局断言：标题栏有 Session log、Git 和两个开关且框不互叠，右侧栏空态有五张卡，surfaces 打开时仍能打开 branch menu。该通道不执行 Electron `-webkit-app-region` 命中。源码与打包 Electron smoke 在打开 surfaces 后，于真实按钮中心发送 `Input.dispatchMouseEvent`，并要求 branch 与 Git 菜单发生变化。
