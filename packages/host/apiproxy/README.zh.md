...
`session.fork` 恰好接受两个互斥锚点之一。`atSeq` 映射到该锚点处或其后的首个 `turn/end`，使消息操作可包含该消息所在的完整轮次；锚点省略或超过末尾时，选择最后一个已完成轮次；若锚点已在日志中，而其所在轮次仍开放，则返回 `fork-unavailable`，不会向较早位置裁剪。`beforeSeq` 则在该锚点事件所属轮次**之前**切断——目标轮（无论已完成还是仍在运行）整体排除，锚点指向或早于首个 `turn/start` 时得到空种子——因此“编辑并重新发送”操作可以 fork 出一个保留此前所有轮次、却不含正在改写消息的子会话。两个锚点同时给出、seq 在日志之外，或锚点落在轮次之间时返回 `fork-unavailable`。发布后的子会话会先继承源会话的种子历史、cwd、日志中最新的 `ModelSelection` 及谱系，再加入源 Workspace；fork 返回值与 `workspace-attach-failed` 的详情都会报告子会话的 `blank` 状态（是否已开始过轮次），让客户端能正确呈现首轮前的空子会话。如果附加到 Workspace 失败，`workspace-attach-failed` 会携带已发布的子会话 id，供客户端对账。[SessionStore fork 决策](../../../.agents/notes/implemented/feature/2026-06-30-session-store-fork-api.md)记录了为何 `atSeq` 锚点要映射到该 `turn/end`。
