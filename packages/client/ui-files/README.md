...
- **The tree does not mutate the workspace** — there is no create, rename, or delete (matching T3's tree). Mention is the row `@` into the composer when a session id is present, not T3 drag-from-tree. Text files that fit the read cap can be edited and saved.
