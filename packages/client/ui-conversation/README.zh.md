...
- **核心 user chrome 不编辑已发送消息**：气泡仍只带时钟和复制；分支只存在于 assistant 回答之下（[决策](../../../.agents/notes/implemented/simplification/2026-08-06-user-bubbles-drop-the-branch-action.md)）。`conversation.chat.user-actions` 与 `conversation.chat.user-editor` 允许插件把最新一条 user 气泡换成就地编辑器；确认时在该消息之前 fork，而不是改写日志（[决策](../../../.agents/notes/implemented/feature/2026-08-15-inline-user-message-edit.md)）。
